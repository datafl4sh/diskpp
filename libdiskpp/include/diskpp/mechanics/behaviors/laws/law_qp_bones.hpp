...
class law_qp_bones {
  ...
  protected:
    ...
    static_tensor< scalar_type, DIM > elastic_modulus( const data_type &data ) const {

        return 2 * data.getMu() * IdentitySymTensor4< scalar_type, DIM >() +
               data.getLambda() * IxI< scalar_type, DIM >();
    }

    static_tensor< scalar_type, 3 > elastic_modulus3D( const data_type &data ) const {

        return 2 * data.getMu() * IdentitySymTensor4< scalar_type, 3 >() +
               data.getLambda() * IxI< scalar_type, 3 >();
    }

    scalar_type sigmaeq( const static_matrix_type3D &dev ) const {
  ...
  public:
    ...
    qprst< scalar_type, DIM > quadrature_point() const { return make_qp( m_point, m_weight ); }

    auto point() const { return m_point; }

    scalar_type weight() const { return m_weight; }

    bool is_plastic() const { return false; }

    static_matrix_type3D getElasticStrain() const { return m_estrain_curr; }

    static_matrix_type3D getPlasticStrain() const { return static_matrix_type3D::Zero(); }

    static_matrix_type getTotalStrain() const { return m_estrain_curr.block( 0, 0, DIM, DIM ); }

    static_matrix_type getTotalStrainPrev() const { return m_estrain_prev.block( 0, 0, DIM, DIM ); }
