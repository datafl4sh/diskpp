...
class LinearIsotropicAndKinematicHardening_qp : public law_qp_bones< T, DIM > {
  ...
  public:
    ...
    LinearIsotropicAndKinematicHardening_qp( const point< scalar_type, DIM > &point,
    ...
    compute_whole3D( const static_matrix_type3D &strain_curr, const data_type &data,
        ...
        if ( m_is_plastic_curr ) {
            const scalar_type dem = 3 * data.getMu() + data.getH() +
                                    scalar_type( 3. ) * data.getK() / scalar_type( 2. );
