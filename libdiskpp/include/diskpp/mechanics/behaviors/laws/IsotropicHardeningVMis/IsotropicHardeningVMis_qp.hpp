...
class IsotropicHardeningVMis_qp : public law_qp_bones< T, DIM > {
  ...
  public:
    IsotropicHardeningVMis_qp()
        : law_qp_bones< T, DIM >(),
          m_pstrain_prev( static_matrix_type3D::Zero() ),
          m_p_prev( scalar_type( 0 ) ),
          m_pstrain_curr( static_matrix_type3D::Zero() ),
          m_p_curr( scalar_type( 0 ) ),
    ...
    bool is_plastic() const { return m_is_plastic_curr; }

    static_matrix_type3D getPlasticStrain() const { return m_pstrain_curr; }

    static_matrix_type getTotalStrain() const {
        return convertMatrix< scalar_type, DIM >( this->m_estrain_curr + m_pstrain_curr );
    }

    static_matrix_type getTotalStrainPrev() const {
        return convertMatrix< scalar_type, DIM >( this->m_estrain_prev + m_pstrain_prev );
    }

    scalar_type getEquivalentPlasticStrain() const { return m_p_curr; }
    ...
    compute_whole3D( const static_matrix_type3D &strain_curr, const data_type &data,
        ...
        const static_matrix_type3D incr_strain =
            strain_curr - convertMatrix3D( this->getTotalStrainPrev() );
        const static_matrix_type3D estrain_trial =
        ...
        if ( nb_point == 0 ) {
            ...
            const scalar_type H0 =
                ( RpCurve[i0 + 1].getRp() - Rp0 ) / ( RpCurve[i0 + 1].getP() - p0 );
            ...
            // std::cout << "se_eq: " << se_eq << ", Rp " << Rp0 + H0 * (m_p_prev - p0) << ", Phi: "
            // << Phi_trial0 << std::endl;

            if ( ( std::abs( se.trace() ) / se.norm() ) > 1E-8 ) {
                // std::cout << std::abs(se.trace()) << ", " << se.norm() << ", " <<
                // (std::abs(se.trace()) / se.norm())
                //           << std::endl;
            ...
            if ( m_is_plastic_curr ) {
                ...
                for ( size_t i = i0 + 1; i < nb_point - 1; i++ ) {
                    const scalar_type eq =
                        RpCurve[i].getRp() - troismu * ( m_p_prev - RpCurve[i].getP() ) - se_eq;
                ...
                const scalar_type H1 =
                    ( RpCurve[i1 + 1].getRp() - Rp1 ) / ( RpCurve[i1 + 1].getP() - p1 );

                const scalar_type Phi_trial1 = se_eq - Rp1 - H1 * ( m_p_prev - p1 );

                const scalar_type dem = troismu + H1;
                const static_matrix_type3D normal =
                    scalar_type( 3. ) * se / ( scalar_type( 2. ) * se_eq );
                const scalar_type delta_p = Phi_trial1 / dem;

                // std::cout << "P: " << m_p_prev << ", Rp: " << Rp1 + H1 * (m_p_prev - p1) << ", H:
                // " << H1
                ...
                if ( tangentmodulus ) {
                    ...
                    const static_tensor< scalar_type, 3 > Is =
                        IdentitySymTensor4< scalar_type, 3 >();
                    const static_tensor< scalar_type, 3 > Pdev =
                        Is - IxI< scalar_type, 3 >() / scalar_type( 3 );
                    const scalar_type mu2 = data.getMu() * data.getMu();

                    Cep += 4 * mu2 * ( delta_p / se_eq - 1.0 / dem ) * nxn -
                           6.0 * mu2 * delta_p / se_eq * Pdev;
                }
            } else {
                // update
                this->m_estrain_curr = estrain_trial;
